...
class CharacterizeDonutImageConfig(pexConfig.Config):
    ...
    doWriteExposure = pexConfig.Field(
        dtype = bool,
        default = True,
        doc = "Write icExp and icExpBackground in addition to icSrc? Ignored "
              "if doWrite False.",
    )
    ...
    checkUnitsParseStrict = pexConfig.Field(
        doc = "Strictness of Astropy unit compatibility check, can be "
              "'raise', 'warn' or 'silent'",
        dtype = str,
        default = "raise",
    )
...
class CharacterizeDonutImageTask(pipeBase.CmdLineTask):
    """!Detect and measure bright sources and use this to estimate background
    of an exposure

    @anchor CharacterizeDonutImageTask_

    @section donut_CharacterizeDonutImage_Contents  Contents

     - @ref donut_CharacterizeDonutImage_Purpose
     - @ref donut_CharacterizeDonutImage_Initialize
     - @ref donut_CharacterizeDonutImage_IO
     - @ref donut_CharacterizeDonutImage_Config
     - @ref donut_CharacterizeDonutImage_Debug


    @section donut_CharacterizeDonutImage_Purpose  Description

    Given an exposure with defects repaired (masked and interpolated over, e.g.
    ...
    @section donut_CharacterizeDonutImage_Initialize  Task initialisation

    @copydoc \_\_init\_\_

    @section donut_CharacterizeDonutImage_IO  Invoking the Task
    ...
    output then it is more direct to call the `characterize` method:

    @section donut_CharacterizeDonutImage_Config  Configuration parameters

    See @ref CharacterizeDonutImageConfig

    @section donut_CharacterizeDonutImage_Debug  Debug variables
    ...
    <dl>
    <dt>frame
    <dd>int: if specified, the frame of first debug image displayed (defaults
        to 1)
    <dt>repair_iter
    <dd>bool; if True display image after each repair in the measure PSF loop
    <dt>background_iter
    <dd>bool; if True display image after each background subtraction in the
        measure PSF loop
    <dt>measure_iter
    <dd>bool; if True display image and sources at the end of each iteration of
        the measure PSF loop.  See @ref lsst.meas.astrom.displayAstrometry for
    ...
    def __init__(self, schema=None, **kwargs):
        ...
        @param[in,out] schema  initial schema (an lsst.afw.table.SourceTable),
                               or None
    ...
    def run(self, dataRef, exposure=None, background=None, doUnpersist=True):
        ...
        This simply unpacks the exposure and passes it to the characterize
        ...
        @param[in,out] exposure  exposure to characterize (an
            ...
            - set psf to that specified by the installSimplePsf task
            - subtract background
        ...
        @param[in,out] background  initial model of background already
            ...
            None if no background has been subtracted, which is typical for
            image characterization.  A refined background model is output.
        @param[in] doUnpersist  if True the exposure is read from the
            repository and the exposure and background arguments must be None;
        ...
        if doUnpersist:
            if exposure is not None or background is not None:
                raise RuntimeError("doUnpersist true; exposure and background "
                                   "must be None")
    ...
    def characterize(self, exposure, exposureIdInfo=None, background=None):
        ...
        @param[in] exposureIdInfo  ID info for exposure (an
            ...
            If not provided, returned SourceCatalog IDs will not be globally
            unique.
        @param[in,out] background  initial model of background already
            subtracted from exposure (an lsst.afw.math.BackgroundList). May be
            None if no background has been subtracted, which is typical for
        ...
        @return pipe_base Struct containing these fields, all from the final
        iteration of detectMeasureAndEstimatePsf:
        - exposure: characterized exposure; image is repaired by interpolating
            over cosmic rays, mask is updated accordingly, and the PSF model is
            set
        ...
        - psfCellSet: spatial cells of PSF candidates (an
        ...
        self.display("repair", exposure=dmeRes.exposure,
    ...
    def detectMeasureAndEstimatePsf(self, exposure, exposureIdInfo, background):
        ...
        @param[in,out] exposure  exposure to characterize (an
            ...
            - subtract background
        @param[in] exposureIdInfo  ID info for exposure (an
            lsst.obs_base.ExposureIdInfo)
        @param[in,out] background  initial model of background already
            subtracted from exposure (an lsst.afw.math.BackgroundList).

        @return pipe_base Struct containing these fields, all from the final
        iteration of detect sources, measure sources and estimate PSF:
        - exposure  characterized exposure; image is repaired by interpolating
            over cosmic rays, mask is updated accordingly, and the PSF model is
            set
        - sourceCat  detected sources (an lsst.afw.table.SourceCatalog)
        ...
        sourceIdFactory = IdFactory.makeSource(
                exposureIdInfo.expId, exposureIdInfo.unusedBits)
        ...
        self.measurement.run(measCat=sourceCat, exposure=exposure,
                             exposureId=exposureIdInfo.expId)
    ...
    def getSchemaCatalogs(self):
        """Return a dict of empty catalogs for each catalog dataset produced by
        this task.
        """
    ...
    def display(self, itemName, exposure, sourceCat=None):
        ...
        displayAstrometry(exposure=exposure, sourceCat=sourceCat,
                          frame=self._frame, pause=False)
        self._frame += 1
